...
ghb_gdc::IndexTable::IndexTable() {
  {
    for (size_t i = 0; i < INDEX_ENTRIES; i++) {
      this->entries[i].trigger_delta = 0;
      this->entries[i].ghb_ptr = 0;
    }
  }
}

ghb_gdc::GlobalHistoryBuffer::GlobalHistoryBuffer() {
  {
    this->head = 0;
    this->generation = 0;
    for (ghb_ptr_t i = 0; i < GHB_ENTRIES; i++) {
      this->entries[i].target_addr = champsim::block_number{0};
      this->entries[i].delta = 0;
      this->entries[i].next = i;
    }
  }
}

bool ghb_gdc::is_valid_ghbe(ghb_ptr_t ptr) {
...
bool ghb_gdc::has_next_ghbe(ghb_ptr_t ptr) {
  return GHB.entries[entry_index(ptr)].next != ptr;
}

ghb_gdc::ghb_ptr_t ghb_gdc::entry_index(ghb_ptr_t ptr) {
  return static_cast<ghb_ptr_t>((ptr & (GHB_PTR_SIZE - 1)) % GHB_ENTRIES);
}

std::size_t ghb_gdc::calc_ghbe_distance(ghb_ptr_t lhs, ghb_ptr_t rhs) {
...
ghb_gdc::ghb_ptr_t ghb_gdc::make_ghb_ptr(ghb_ptr_t ptr) {
  return static_cast<ghb_ptr_t>(entry_index(ptr) | (GHB.generation << GHB_PTR_BITS));
}

ghb_gdc::ghb_ptr_t ghb_gdc::previous_ghb_ptr(ghb_ptr_t ptr) {
...
void ghb_gdc::prefetcher_initialize() {
  std::cout << "[GHB_DELTA] Initialized Index Table with INDEX_ENTRIES: " << INDEX_ENTRIES << std::endl;
  std::cout << "[GHB_DELTA] Initialized GHB with GHB_ENTRIES: " << GHB_ENTRIES << std::endl;
...
uint64_t ghb_gdc::fmix64 ( uint64_t k ) {
  k ^= k >> 33;
...
std::size_t ghb_gdc::get_hash(delta_t delta) {
  uint64_t mixed_bits = fmix64(static_cast<uint64_t>(delta));
  return static_cast<std::size_t>(mixed_bits % INDEX_ENTRIES);
}

uint32_t ghb_gdc::prefetcher_cache_operate(champsim::address addr, champsim::address ip, bool cache_hit,
                                             bool useful_prefetch, access_type type, uint32_t metadata_in)
{
  ...
  if (!has_last_miss) {
    last_miss_addr = block_addr;
    has_last_miss = true;
    return metadata_in;
  }

  const delta_t delta = static_cast<delta_t>(block_addr.to<uint64_t>() - last_miss_addr.to<uint64_t>());
  last_miss_addr = block_addr;

  const size_t hash = get_hash(delta);
  const bool hash_collision = IT.entries[hash].trigger_delta != delta;
  const ghb_ptr_t head = GHB.head;
  const ghb_ptr_t head_idx = entry_index(GHB.head);

  GHB.entries[head_idx].target_addr = block_addr;
  GHB.entries[head_idx].delta = delta;
  GHB.entries[head_idx].next = make_ghb_ptr(hash_collision ? head : IT.entries[hash].ghb_ptr);

  IT.entries[hash].trigger_delta = delta;
...
void ghb_gdc::prefetcher_cycle_operate() {
